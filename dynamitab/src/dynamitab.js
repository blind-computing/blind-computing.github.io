...
class TabView {
  ...
  get tablist() {
    ...
    for (var i = 0; i < this.tabs.length; i++) {
      var child = this.tabs[i].get_tab_element(
        this.tabs[i].id == this.default_tab,
      );
  ...
  get tabview() {
    ...
    for (var i = 0; i < this.tabs.length; i++) {
      var panel = this.tabs[i].get_panel_element(
        this.tabs[i].id == this.default_tab,
      );
  ...
  createTab(title, description = undefined, content = undefined) {
    this.tabs.push(
      new Tab(this, this.tabs.length, title, description, content),
    );
